...
export function Tag({ text, tone, color }: { text: string; tone?: 'red' | 'gold' | 'plain'; color?: string }) {
  const bg = color ? `${color}26` : tone === 'red' ? colors.accentSoft : tone === 'gold' ? colors.accentSoft : colors.cardAlt;
  const fg = color ? color : tone === 'red' ? colors.accentDeep : tone === 'gold' ? colors.accentDeep : colors.inkSoft;
  return (
...
export function ProGate({ title, body, onPress }: { title: string; body: string; onPress: () => void }) {
  return (
    <Card style={{ marginTop: 16, alignItems: 'flex-start' }}>
      <Tag text="CAT CLUB" tone="gold" />
      <Text style={[type.h2, { marginTop: 10 }]}>{title}</Text>
      <Text style={[type.bodySoft, { marginTop: 6 }]}>{body}</Text>
      <PrimaryButton title="Join the Cat Club" onPress={onPress} style={{ marginTop: 16, alignSelf: 'stretch' }} />
    </Card>
  );
}
